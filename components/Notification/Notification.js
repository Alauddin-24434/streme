...
const NotificationMenu = () => {
  ...
  const handleClick = (event) => {
    ...
    axios.patch(`http://localhost:5000/notifications/openNotify?email=${userInfo?.email}`)
      .then(res => {
        if (res.data) {
  ...
  const handleNotifyRead = (id) => {
    axios.patch(`http://localhost:5000/notifications/completeRead/${id}?email=${userInfo?.email}`)
      .then(res => {
        if (res.data) {
  ...
  useEffect(() => {
    if (userInfo?.email) {
      axios.get(`http://localhost:5000/notifications?email=${userInfo?.email}`)
        .then(res => {
  ...
  useEffect(() => {
    if (userInfo?.email) {
      axios.get(`http://localhost:5000/notifications/read?email=${userInfo?.email}`)
        .then(res => {
