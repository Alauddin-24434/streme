...
interface BodyChildrenProps {
  ...
  onCurrentVisibleStatus?: (id: string, status: string) => void;
  searchQuery: string;
  setSearchQuery: (query: string) => void;
}

const BodyChildren: React.FC<BodyChildrenProps> = ({
  dbutton,
  fetchEpisodes,
  fetchMovies,
  fetchShows,
        ...
        <table className="min-w-full table-auto text-black border border-collapse">
          <thead>
            <tr className='text-white bg-[#09526C]'>
              ...
              {tableData.some(row => row.season) && (
                <th className="py-2 px-4 border border-gray-300 font-bold text-sm text-center lg:flex-1">Season</th>
              )}
          ...
          <tbody>
            {currentData.map((row, index) => (
              <tr key={row._id} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                <td className="py-2 px-2 border border-black text-center lg:w-1/4">
                  {row.title?.slice(0, 20) || row.userName?.slice(0, 20) || 'N/A'}
                </td>
                ...
                {row.season && (
                  <td className="py-2 px-2 border border-black text-center lg:flex-1">{row.season}</td>
                )}
                <td className="py-2 px-2 border border-black text-center lg:flex-1">
                  {onCurrentVisibleStatus && (
                    <MdVisibilityOff
