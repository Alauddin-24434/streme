...
const ShowsPage: React.FC = () => {
  ...
  useEffect(() => {
    fetchShows();
  }, [searchQuery]); // Only fetch on search query change, not on every state update

  const handleCurrentVisibilityStatus = async (showId: string, currentStatus: string) => {
    try {
      await axios.put(`https://endgame-team-server.vercel.app/latestShows/${showId}`, {
  ...
  const handleDeleteData = async (showId: string) => {
    try {
